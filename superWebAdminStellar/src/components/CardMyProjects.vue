...
import TableMyProjects from "@/components/TableMyProjects.vue";
import {chartOptions, formatDateFromServer, formatNumber} from "@/functions";
...
export default class CardMyProjects extends Vue {
  ...
  beforeMount () {
    ...
    UserServices.getDataTablePortfolio(this.user_id)
        .then(response => {
          ...
          for (let element of data.list_invest) {
            ...
            element.slug = '/InvestProjects/debt/' + element.id
