...
export default class CardTotalValue extends Vue {
  ...
  mounted () {
    ...
    if (this.type === "invests") {
      InvestServices.getInvestLastInvest(this.invest_id, this.user_id, this.invest_child_id)
          .then(response => {
      ...
      InvestServices.getInvestLastProfits(this.invest_id, this.user_id, this.invest_child_id)
          .then(response => {
            let data = response.data
            this.total_value = data.value
            this.percentage = data.percentage
      ...
      InvestServices.getInvestLastBalance(this.invest_id, this.user_id, this.invest_child_id)
          .then(response => {
            let data = response.data
            this.total_value = data.value
            this.percentage = data.percentage
